...
class Migrator
{
    ...
    /**
     * The places for the current operation.
     *
     * @var array
     */
    protected $places = [];
    ...
    public function runMigrationList($migrations, array $options = [])
    {
        ...
        // aren't, we will just make a place of it to the developer so they're aware
        // that all of the migrations have been run against this database system.
        if (count($migrations) == 0) {
            $this->place('<info>Nothing to migrate.</info>');
    ...
    protected function runUp($file, $batch, $pretend)
    {
        ...
        $this->place("<info>Migrated:</info> {$file}");
    }
    ...
    public function rollback($paths = [], array $options = [])
    {
        ...
        if ($count === 0) {
            $this->place('<info>Nothing to rollback.</info>');
        } else {
    ...
    protected function runDown($file, $migration, $pretend)
    {
        ...
        $this->place("<info>Rolled back:</info> {$file}");
    }
    ...
    protected function pretendToRun($migration, $method)
    {
        foreach ($this->getQueries($migration, $method) as $query) {
            ...
            $this->place("<info>{$name}:</info> {$query['query']}");
        }
    }
    ...
    /**
     * Raise a place event for the migrator.
     *
     * @param  string  $message
     * @return void
     */
    protected function place($message)
    {
        $this->places[] = $message;
    }

    /**
     * Get the places for the last operation.
     *
     * @return array
     */
